...
describe('CreatePopVersionUseCase', () => {
  ...
  it('should throw NotFoundException when the POP does not exist', async () => {
    ...
    await expect(useCase.execute('proj1', 'missing', { createdById: 'u1' })).rejects.toThrow(NotFoundException);
    expect(repo.createVersion).not.toHaveBeenCalled();
  });

  it('should reject a POP from a different project (anti-IDOR)', async () => {
    await expect(useCase.execute('OTHER_PROJECT', 'pop1', { createdById: 'u1' })).rejects.toThrow(ForbiddenException);
    expect(repo.createVersion).not.toHaveBeenCalled();
  });

  it('should create a new version — never mutate an existing one', async () => {
    await useCase.execute('proj1', 'pop1', { changeNotes: 'ajuste no passo 3', createdById: 'u1' });
