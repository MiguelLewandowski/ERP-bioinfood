...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  TODO:        { label: 'A fazer',       color: '#575756' },
  IN_PROGRESS: { label: 'Em andamento',  color: '#147F23' },
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDragging && 'opacity-40 bg-[#86C175]/10',
      )}
    >
      <button {...attributes} {...listeners} className="text-gray-400 hover:text-[#575756] cursor-grab active:cursor-grabbing">
        <GripVertical size={14} />
      </button>
      <div>
        <p className="text-sm font-medium text-[#1D1D1B] leading-snug">{task.title}</p>
        {task.description && <p className="text-xs text-[#706F6F] truncate max-w-sm">{task.description}</p>}
      </div>
      ...
      <span className="text-sm font-semibold text-right text-[#575756]">{task.storyPoints ?? '—'}</span>
      <span className="text-xs text-right text-[#706F6F]">
        {task.dueDate ? new Date(task.dueDate).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }) : '—'}
      </span>
      <button
        onClick={() => onEdit(task)}
        className="opacity-0 group-hover:opacity-100 p-1.5 rounded-md hover:bg-gray-200 text-[#706F6F] hover:text-[#147F23] transition-all"
        title="Editar tarefa"
      >
        <Pencil size={13} />
      </button>
