    ...
    <div className="overflow-x-auto">
      <div className="inline-block min-w-[480px]">
        <div className="flex items-center mb-2">
          ...
          <div className="flex-1 text-center text-xs font-semibold text-[#575756] mb-1">INTERESSE →</div>
        </div>
        <div className="flex items-center mb-1">
          <div className="w-24 shrink-0" />
          {LEVELS.map((l) => (
            <div key={l} className="flex-1 text-center text-[10px] text-[#706F6F] px-0.5">
              {LEVEL_LABELS[l]}
            </div>
          ))}
        </div>
        ...
        {[...LEVELS].reverse().map((influence) => {
          ...
          return (
            <div key={influence} className="flex items-center mb-1">
              <div className="w-24 shrink-0 text-[10px] text-[#706F6F] text-right pr-2 leading-tight">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      'hover:opacity-80 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#52B552]',
                    )}
        ...
        <div className="flex items-center mt-1">
          <div className="w-24 shrink-0" />
          <div className="flex-1 text-center text-xs font-semibold text-[#575756]">← PODER (INFLUÊNCIA)</div>
        </div>
        ...
        <div className="flex gap-3 mt-4 flex-wrap">
          {(Object.keys(QUADRANT_LABELS) as PowerInterestQuadrant[]).map((q) => (
            <div key={q} className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-[#706F6F]">{QUADRANT_LABELS[q]}</span>
            </div>
          ))}
        </div>
      </div>
